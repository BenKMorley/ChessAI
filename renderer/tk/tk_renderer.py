...
shape_dict = {'bishop': bishop_shape, 'knight': knight_shape,
              'king': king_shape, 'queen': queen_shape, 'rook': rook_shape}
...
class BoardGuiTk(tk.Frame):
    ...
    def __init__(self, parent, chessboard, square_size=64, colour1="#F5F5DC", colour2="#825A50"):
        ...
        self.button_reset = tk.Button(
            self.statusbar, text="New", fg="black", command=self.draw_pieces)
    ...
    def draw_piece(self, piece, position):
        ...
        if piece_type in shape_dict:
            ...
            points = [[point[0] * size + x1c, -point[1] * size + y1c]
                      for point in points]
        ...
        for i in range(8):
            for j in range(8):
                if self.chessboard.get_positions()[i, j] != '':
                    self.draw_piece(
                        self.chessboard.get_positions()[i, j], (j, 7-i))
